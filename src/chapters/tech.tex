...
Flutter并不是唯一一个支持Android和iOS平台的跨平台应用程序开发框架。在本项目的选型过程中也考虑了其他几个经常被与Flutter进行比较的同类框架，包括React Native\cite{ReactNativeLearn}、Xamarin\cite{XamarinForms2023}、Ionic Creator\cite{IonicFrameworkCrossPlatform}。
...
PyQtDeploy\cite{RiverbankComputingIntroduction}、QPython\cite{QPythonPythonAndroid}、Termux\cite{TermuxApplication2023}等工具可以用于在移动端执行Python代码，但未提供可供Java、Dart等语言调用的接口，因此无法在本项目中使用。
