...
\begin{document}
    ...
    \newcommand*{\app}{面向移动终端的可穿戴动态心电图的智能监测应用}
...
% 设置正文编号
    ...
    \section{\app 主要工作}\label{sec:work}
    \todo{todo}
    ...
    \chapter{\app 需求分析}\label{ch:requirement}
    \todo{todo}

    本应用的目标用户是佩戴可穿戴心电信号监测设备的院外患者，以中老年人为主。


    \chapter{\app 设计}\label{ch:design}
    \todo{todo}


    \chapter{\app 实现}\label{ch:implement}
    \todo{todo}


    \chapter{\app 测试}\label{ch:test}
    \todo{测试环境和测试数据的情况等。}
